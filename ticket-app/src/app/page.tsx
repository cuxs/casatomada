...
export default function HomePage() {
  ...
  function copyAlias() {
    navigator.clipboard.writeText(EVENT_CONFIG.alias).then(() => {
      setAliasCopied(true);
  ...
  function copyPhone() {
    navigator.clipboard.writeText(EVENT_CONFIG.phone).then(() => {
      setPhoneCopied(true);
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gray-50 p-3 rounded-xl border border-gray-100">
            <div className="min-w-0">
              ...
              <p className="text-gray-900 font-mono font-medium text-base break-all">{EVENT_CONFIG.alias}</p>
            </div>
            <button
              onClick={copyAlias}
          ...
          >
            {phoneCopied ? "¡Copiado!" : `Copiar teléfono para transferir (${EVENT_CONFIG.phone})`}
          </button>
        </div>
